...
async function insertUser(name, surname1, surname2, user_name, email, user_type, password){
    ...
    try{
      ...
      const res = await conn.query(sql);
            ...
            console.log('Va a mostrar errores ' + err);
            mensaje = mostrarError(err.errno, err.text, errores);
            console.log('Ojo al dato');
            console.table(mensaje);
            //console.log(err);
            //console.log(err.text);
            //console.log("No se ha podido realizar la inserción"); 
            return 'Cacho guarro:' + mensaje;
    } finally {
      //conn.end();
    }
    return 'Final normal';
...
function mostrarError(error, texto, errores){
  let posicion = 0;
  let posicionFin = 0;
  let patron = "";
  let campo = "";
    ...
    case 1062:
      ...
      switch (campo){
        case ("'user_name'"):
          ...
          errores.push('Ya existe un usuario registrado con ese nombre de usuario');
          break;
        case ("'email'"):
          console.log('Ya existe un usuario registrado con ese email');
          errores.push('Ya existe un usuario registrado con ese email');
          break;
      }
      break;
    case 4025:
      ...
      switch (campo){
        case ("`CHK_Premium`"):
          ...
          errores.push('Para registrar un usuario premium, hay que introducir un email');
          break;
      }
      break;
  }
...
module.exports = {pool1, pool2, insertUser, errores, getErrores};